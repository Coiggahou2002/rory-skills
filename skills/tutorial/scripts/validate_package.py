...
class Validator:
    ...
    def validate_dependencies(self) -> None:
        ...
        if shutil.which("mmdc"):
            self.pass_("mmdc (Mermaid CLI) is available")
        else:
            self.fail("mmdc is missing; install with: npm install -g @mermaid-js/mermaid-cli")
    ...
    def validate_visuals(self) -> None:
        ...
        for index, chapter in enumerate(chapters, start=1):
            ...
            if not chapter.get("mermaid"):
                self.warn(f"{visual_id} has no mermaid code")

            mmd = self.package_dir / "visuals" / f"{visual_id}.mmd"
            svg = self.package_dir / "visuals" / f"{visual_id}.svg"
            png = self.package_dir / "assets/screenshots" / f"{visual_id}.png"
            if mmd.exists():
                self.pass_(f"Mermaid source exists for {visual_id}")
            else:
                self.warn(f"Missing .mmd file for {visual_id}")
            if svg.exists():
    ...
    def validate_png_size(self, png: Path, visual_id: str) -> None:
        ...
        try:
            ...
            return
        if width >= 1200:
            self.pass_(f"PNG for {visual_id} is high-resolution ({width}x{height})")
        else:
            self.warn(f"PNG for {visual_id} may be low resolution ({width}x{height})")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate a generated tutorial output package.")
    parser.add_argument("package_dir", help="Generated tutorial package directory, usually outputs/tutorials/<topic-slug>.")
